...
<span class="mx-2">/</span>
<span class="text-gray-900 dark:text-gray-100 font-medium">{{ __('admin.edit_user') }}</span>
@endsection

@section('content')
<div class="space-y-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">{{ __('admin.edit_user') }}: {{ $user->full_name }}</h1>
    ...
    <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="flex items-center gap-4">
            ...
            <span class="text-sm text-gray-500">{{ __('admin.avatar') }}</span>
        </div>
        @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('admin.password') }} <span class="text-xs text-gray-400">({{ __('admin.leave_blank') ?? 'boş bırakın' }})</span></label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <div>
                ...
                <input type="text" id="timezone" name="timezone" value="{{ old('timezone', $user->timezone) }}"
                       class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-[#1A3A5C] bg-white dark:bg-[#0A1628] text-sm focus:ring-[#0B6AB2] focus:border-[#0B6AB2]">
            </div>
            <div>
        ...
        <div class="flex items-center gap-3 pt-4 border-t border-gray-100 dark:border-[#1A3A5C]">
            ...
            <a href="{{ route('admin.users.show', $user) }}" class="px-6 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 transition-colors">
                {{ __('admin.cancel') }}
            </a>
        </div>
    </form>
