...
@section('content')
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.new_page') }}</h1>
        ...
        <form action="{{ route('admin.pages.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="content[{{ $locale }}]" rows="8"
                                class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628] text-gray-900 dark:text-white focus:ring-2 focus:ring-[#0B6AB2]">{{ old("content.{$locale}") }}</textarea>
                        </div>
                        <div>
                            <label
                            ...
                            <textarea name="excerpt[{{ $locale }}]" rows="2"
                                class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628] text-gray-900 dark:text-white">{{ old("excerpt.{$locale}") }}</textarea>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Meta
                                    Title</label>
                                <input type="text" name="meta_title[{{ $locale }}]" value="{{ old("meta_title.{$locale}") }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="meta_description[{{ $locale }}]"
                                    value="{{ old("meta_description.{$locale}") }}"
                                    class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">
                            </div>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="template" value="{{ old('template', 'default') }}"
                        class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">
                </div>
                <div>
                    <label
