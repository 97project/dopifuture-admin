...
@section('content')
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.edit_page') }}:
        ...
        <form action="{{ route('admin.pages.update', $page) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title[{{ $locale }}]"
                                value="{{ old("title.{$locale}", $page->title[$locale] ?? '') }}" required
                                class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628] text-gray-900 dark:text-white focus:ring-2 focus:ring-[#0B6AB2]">
                        </div>
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('admin.content') }}</label>
                            <textarea name="content[{{ $locale }}]" rows="8"
                                class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628] text-gray-900 dark:text-white focus:ring-2 focus:ring-[#0B6AB2]">{{ old("content.{$locale}", $page->content[$locale] ?? '') }}</textarea>
                        </div>
                        <div>
                            <label
                            ...
                            <textarea name="excerpt[{{ $locale }}]" rows="2"
                                class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">{{ old("excerpt.{$locale}", $page->excerpt[$locale] ?? '') }}</textarea>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Meta
                                    Title</label>
                                <input type="text" name="meta_title[{{ $locale }}]"
                                    value="{{ old("meta_title.{$locale}", $page->meta_title[$locale] ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Meta
                                    Description</label>
                                <input type="text" name="meta_description[{{ $locale }}]"
                                    value="{{ old("meta_description.{$locale}", $page->meta_description[$locale] ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">
                            </div>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="archived" @selected($page->status === 'archived')>Archived</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    @if($page->featured_image)
                        <img src="{{ asset('storage/' . $page->featured_image) }}" class="w-20 h-20 object-cover rounded mb-2">
                    @endif
            ...
            <div class="flex items-center gap-4">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="is_homepage" value="1" {{ $page->is_homepage ? 'checked' : '' }}
                        class="rounded border-gray-300 text-[#0B6AB2] focus:ring-[#0B6AB2]">
                    <span class="text-sm text-gray-700 dark:text-gray-300">{{ __('admin.homepage') }}</span>
                </label>
            </div>

            <div class="flex gap-3 pt-4 border-t border-gray-100 dark:border-[#1A3A5C]">
                <button type="submit"
                    class="px-6 py-2.5 bg-[#0B6AB2] hover:bg-[#13398E] text-white font-medium rounded-lg transition-colors">{{ __('admin.save') }}</button>
                <a href="{{ route('admin.pages.index') }}"
                    class="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 dark:bg-[#0A1628] dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 font-medium rounded-lg transition-colors">{{ __('admin.cancel') }}</a>
            </div>
        </form>
    </div>
@endsection