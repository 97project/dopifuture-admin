...
@section('content')
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.edit_faq') }}</h1>
            ...
            <div>
                ...
                <select name="faq_category_id" required
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" @selected($faq->faq_category_id == $cat->id)>
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="text" name="question[{{ $locale }}]"
                                value="{{ old("question.{$locale}", $faq->question[$locale] ?? '') }}" required
                        ...
                        <div>
                            ...
                            <textarea name="answer[{{ $locale }}]" rows="4" required
                                class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628] text-gray-900 dark:text-white">{{ old("answer.{$locale}", $faq->answer[$locale] ?? '') }}</textarea>
                        </div>
                    </div>
                </fieldset>
            @endforeach
