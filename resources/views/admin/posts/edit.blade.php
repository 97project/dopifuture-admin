...
@section('content')
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.edit_post') }}:
        ...
        <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title[{{ $locale }}]"
                                value="{{ old("title.{$locale}", $post->title[$locale] ?? '') }}" required
                        ...
                        <div>
                            ...
                            <textarea name="content[{{ $locale }}]" rows="10"
                                class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628] text-gray-900 dark:text-white">{{ old("content.{$locale}", $post->content[$locale] ?? '') }}</textarea>
                        </div>
                        <div>
                            <label
                            ...
                            <textarea name="excerpt[{{ $locale }}]" rows="2"
                                class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">{{ old("excerpt.{$locale}", $post->excerpt[$locale] ?? '') }}</textarea>
                        </div>
                    </div>
                </fieldset>
            @endforeach
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    @if($post->featured_image)
                        <img src="{{ asset('storage/' . $post->featured_image) }}" class="w-20 h-20 object-cover rounded mb-2">
                    @endif
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach($categories as $cat)
                        <label
                            ...
                            <input type="checkbox" name="categories[]" value="{{ $cat->id }}" {{ $post->categories->contains($cat->id) ? 'checked' : '' }}
            ...
            <div>
                ...
                <input type="text" name="tags_input"
                    value="{{ $post->tags->pluck('name')->map(fn($n) => is_array($n) ? ($n[app()->getLocale()] ?? reset($n)) : $n)->join(', ') }}"
                    class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">
