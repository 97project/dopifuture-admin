...
@section('breadcrumb')
    <div class="flex items-center gap-2">
        ...
        <span class="text-gray-900 dark:text-gray-100 font-semibold">Seat Requests</span>
    </div>
@endsection
...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-[#0E2442]/50 rounded-xl border border-gray-100 dark:border-[#1A3A5C] overflow-hidden">
        <div class="p-5 border-b border-gray-100 dark:border-[#1A3A5C]">
            <h3 class="text-sm font-bold text-gray-900 dark:text-white">School Seat Requests</h3>
            <p class="text-xs text-gray-400 mt-1">Review and approve or reject seat allocation requests from school administrators.</p>
        </div>
                            ...
                            @if($req->status === 'pending')
                                <div class="flex items-center gap-2 justify-end">
                                    <form method="POST" action="{{ route('admin.seat-requests.approve', $req) }}" class="inline">
                                        ...
                                        <button type="submit" onclick="return confirm('Approve +{{ $req->requested_seats }} seats for {{ $req->school->name ?? '' }}?')"
                                    ...
                                    <form method="POST" action="{{ route('admin.seat-requests.reject', $req) }}" class="inline">
                                        @csrf
                                        <button type="submit" onclick="return confirm('Reject this seat request?')"
