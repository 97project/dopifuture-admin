...
<div class="mb-6">
    ...
    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('admin.rep_system_wide') }}</p>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
        ...
        <div class="text-xs text-gray-500 mt-1">{{ __('admin.rep_total_users') }}</div>
    </div>
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
        <div class="text-3xl font-extrabold text-emerald-500">{{ $overview['total_students'] }}</div>
        <div class="text-xs text-gray-500 mt-1">{{ __('admin.rep_total_students') }}</div>
    </div>
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
        <div class="text-3xl font-extrabold text-blue-500">{{ $apps->count() }}</div>
        <div class="text-xs text-gray-500 mt-1">{{ __('admin.rep_applications') }}</div>
    </div>
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
        <div class="text-3xl font-extrabold text-amber-500">{{ $schools->count() }}</div>
        <div class="text-xs text-gray-500 mt-1">{{ __('admin.auto_schools') }}</div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">{{ __('admin.rep_app_usage') }}</h3>
        <canvas id="appUsageChart" height="200"></canvas>
    </div>
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">{{ __('admin.rep_completion_status') }}</h3>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
    ...
    <a href="{{ route('admin.reports.app', $stat['app']->slug) }}" class="block bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 hover:border-blue-400 dark:hover:border-blue-600 transition-all group">
        <div class="flex justify-between items-center mb-3">
            ...
            <span class="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-semibold rounded-md">{{ $stat['total_users'] }} {{ __('admin.rep_users_sm') }}</span>
        </div>
...
<div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-800">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white">🏫 {{ __('admin.rep_school_dist') }}</h3>
    </div>
    <table class="w-full">
        <thead>
            <tr class="bg-gray-50 dark:bg-gray-800/50">
                <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('admin.rep_school') }}</th>
                <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('admin.rep_users') }}</th>
                <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('admin.rep_classes') }}</th>
                <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('admin.auto_licenses') }}</th>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                ...
                <td class="px-5 py-3 text-right">
                    <a href="{{ route('admin.reports.school', $s) }}" class="text-xs font-medium text-blue-600 dark:text-blue-400 hover:underline">{{ __('admin.rep_report') }} →</a>
                </td>
            </tr>
            @endforeach
        </tbody>
...
new Chart(document.getElementById('completionChart'), {
    ...
    data: {
        labels: {!! json_encode($overview['app_stats']->pluck('app.name')) !!},
        datasets: [
            { label: '{{ __('admin.rep_completed') }}', data: {!! json_encode($overview['app_stats']->pluck('completed')) !!}, backgroundColor: isDark ? 'rgba(74,222,128,0.7)' : 'rgba(34,197,94,0.7)', borderRadius: 6 },
            { label: '{{ __('admin.rep_in_progress') }}', data: {!! json_encode($overview['app_stats']->pluck('in_progress')) !!}, backgroundColor: isDark ? 'rgba(59,130,246,0.7)' : 'rgba(37,99,235,0.7)', borderRadius: 6 },
        ]
    },
