...
@section('title', isset($translation) ? __('admin.edit_translation') : __('admin.new_translation'))
@section('content')
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">
            {{ isset($translation) ? __('admin.edit_translation') : __('admin.new_translation') }}
        </h1>
        <form
            action="{{ isset($translation) ? route('admin.translations.update', $translation) : route('admin.translations.store') }}"
            method="POST"
            class="bg-white dark:bg-[#0E2442]/50 rounded-xl border border-gray-100 dark:border-[#1A3A5C] p-6 space-y-5">
            @csrf
            @if(isset($translation)) @method('PUT') @endif

            @if(!isset($translation))
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label for="group"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('admin.group') }}
                            *</label>
                        <input type="text" id="group" name="group" value="{{ old('group') }}" required
                    ...
                    <div>
                        <label for="key"
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('admin.key') }}
                            *</label>
            ...
            @foreach($languages as $lang)
                <div>
                    ...
                    <textarea id="value_{{ $lang->code }}" name="values[{{ $lang->code }}]" rows="2"
                        class="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-[#1A3A5C] bg-white dark:bg-[#0A1628] text-sm focus:ring-[#0B6AB2] focus:border-[#0B6AB2]">{{ old("values.{$lang->code}", isset($allTranslations) ? ($allTranslations[$lang->code]->value ?? '') : '') }}</textarea>
                </div>
            @endforeach
