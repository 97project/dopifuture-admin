...
@section('title', __('admin.new_form'))
@section('content')
    <div class="space-y-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.new_form') }}</h1>
        ...
        <form action="{{ route('admin.forms.store') }}" method="POST" id="form-builder"
            ...
            <div>
                ...
                <textarea name="description" rows="2"
                    class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">{{ old('description') }}</textarea>
            </div>
            ...
            <div>
                ...
                <div id="fields-container" class="space-y-3">
                    {{-- Fields are added dynamically via JS --}}
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="notification_emails_text" value="{{ old('notification_emails_text') }}"
                    class="w-full px-3 py-2 border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]"
            ...
            <div class="flex gap-4">
                ...
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="requires_captcha" value="1" checked
                        class="rounded border-gray-300 text-[#0B6AB2]">
    ...
    <script>
                        ...
                        <div class="border border-gray-100 dark:border-[#1A3A5C] rounded-lg p-4 relative" id="field-${fieldIndex}">
                            ...
                            <div class="grid grid-cols-3 gap-3 mb-3">
                                <div>
                                    ...
                                    <input type="text" name="fields[${fieldIndex}][name]" required class="w-full px-2 py-1.5 text-sm border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]" placeholder="e.g. email">
                                </div>
                                <div>
                                ...
                                <div class="flex items-end">
                                    <label class="flex items-center gap-1">
                                        <input type="checkbox" name="fields[${fieldIndex}][required]" value="1" class="rounded border-gray-300 text-[#0B6AB2]">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="text" name="fields[${fieldIndex}][label][tr]" required class="w-full px-2 py-1.5 text-sm border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-500 mb-1">Label (EN) *</label>
                                    <input type="text" name="fields[${fieldIndex}][label][en]" required class="w-full px-2 py-1.5 text-sm border border-gray-200 dark:border-[#1A3A5C] rounded-lg bg-white dark:bg-[#0A1628]">
                                </div>
                            </div>
                        </div>
                    `;
        ...
        // Convert notification emails text to array
