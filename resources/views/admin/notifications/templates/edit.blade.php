...
@section('title', __('admin.edit_template'))
@section('content')
    <div class="space-y-6">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('admin.edit_template') }}</h1>
        </div>
        ...
        <form action="{{ route('admin.notification-templates.update', $template) }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="key" value="{{ old('key', $template->key) }}" required
                    ...
                    @foreach (['database', 'fcm', 'mail'] as $channel)
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="channels[]" value="{{ $channel }}" class="rounded border-gray-300"
                                {{ in_array($channel, old('channels', $template->channels ?? ['database'])) ? 'checked' : '' }}>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="px-6 py-2 bg-[#0B6AB2] hover:bg-[#13398E] text-white text-sm rounded-lg">{{ __('admin.update') }}</button>
