...
@section('content')
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding:20px 24px;border-radius:16px;background:linear-gradient(135deg,#059669,#10B981);color:#fff;">
            <div style="display:flex;flex-direction:column;gap:4px;">
                ...
                <span style="font-size:13px;font-weight:500;opacity:0.9;">{{ $isTr ? 'Ortalama Giriş Sayısı' : 'Average Login Count' }}</span>
            </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding:20px 24px;border-radius:16px;background:linear-gradient(135deg,#0284C7,#38BDF8);color:#fff;">
            <div style="display:flex;flex-direction:column;gap:4px;">
                ...
                <span style="font-size:13px;font-weight:500;opacity:0.9;">{{ $isTr ? 'Ortalama Giriş Süresi' : 'Average Login Duration' }}</span>
            </div>
    ...
    <div class="dp-card" style="padding:0;">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:16px 20px;border-top:1px solid var(--color-row-border);margin-top:8px;">
            <button style="padding:8px 20px;border:1px solid var(--color-row-border);border-radius:8px;background:#fff;cursor:pointer;font-size:13px;font-weight:500;color:var(--color-txt-sec);font-family:'Nunito',sans-serif;">{{ $isTr ? 'Önceki' : 'Previous' }}</button>
            <span style="font-size:13px;color:var(--color-txt-muted);font-family:'Nunito',sans-serif;">{{ $isTr ? 'Sayfa' : 'Page' }} 1 {{ $isTr ? '/' : 'of' }} 3</span>
            <button style="padding:8px 20px;border:1px solid var(--color-row-border);border-radius:8px;background:#fff;cursor:pointer;font-size:13px;font-weight:500;color:var(--color-txt-sec);font-family:'Nunito',sans-serif;">{{ $isTr ? 'Sonraki' : 'Next' }}</button>
        </div>
    </div>
