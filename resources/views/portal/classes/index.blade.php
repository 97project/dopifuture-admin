    ...
    <div class="dp-card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:12px;">
            <div>
                ...
                <p style="font-size:13px;color:var(--text-muted);margin:0;">{{ __('portal.manage_classes_subtitle') }}</p>
            </div>
            <div style="display:flex;gap:8px;align-items:center;">
                <form style="display:flex;gap:8px;">
                    <div class="dp-search" style="width:220px;">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('portal.search_class') }}">
                    </div>
                ...
                @if(auth()->user()->hasAnyRole(['super-admin','admin','license-manager','school-admin','school-principal']))
                    <a href="{{ route('portal.classes.create') }}" class="dp-btn">+ {{ __('portal.new_class') }}</a>
                @endif
            </div>
        </div>
                    ...
                    <tr>
                        ...
                        <td style="text-align:right;">
                            <div style="display:flex;gap:4px;justify-content:flex-end;">
                                <a href="{{ route('portal.classes.show', $cls) }}" class="dp-action dp-action-view" title="{{ __('portal.detail') }}">
                                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('portal.classes.edit', $cls) }}" class="dp-action dp-action-edit" title="{{ __('admin.edit') }}">
                                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                @if(auth()->user()->hasAnyRole(['school-admin','school-principal']))
                                <form action="{{ route('portal.classes.destroy', $cls) }}" method="POST" style="display:inline;" onsubmit="return confirm('{{ __("portal.confirm_delete_class") }}')">
                                    @csrf @method('DELETE')
